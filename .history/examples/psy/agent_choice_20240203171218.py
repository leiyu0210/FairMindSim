...
client = ZhipuAI(api_key="83e5bc58555d8bac289e27bac50f8afc.Khk1JjCxb8MJN8Mi")  # 填写您自己的APIKey
...
def get_glm_res(sys_prompt, user_prompt, model_name="glm-4"):
...
def get_res(
    ...
    if model_type in [
        ...
        sys_prompt = role.content + extra_prompt
        user_prompt = first_message.content
        final_res = get_glm_res(sys_prompt, user_prompt, model_type.value)
    else:
    ...
    return (res, content, structured_dict, input_content, sys_prompt, user_prompt)
...
def gen_character_res(
    ...
    while cha_num < len(all_chara):
        ...
        try:
            ont_res, dialog, structured_dict, input_content, sys_prompt, user_prompt = get_res(
                role_message,
                message,
                cri_agent,
                model_type,
