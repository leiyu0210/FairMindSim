...
def load_experiment_data():
    """Load experiment data files at run time.

    These files are not shipped with the repository (see README Data
    Availability). Importing this module without them is allowed; calling
    ``load_experiment_data()`` will raise if anything is missing.
    """
    return {
        "all_chara": _load_json("data/characters.json"),
        "allocation_ex": _load_json("data/allocation_ex.json"),
        "allocation_se": _load_json("data/allocation_se.json"),
        "need_run_ids": _load_json("data/no_format_ids_ex_gpt-3.5-turbo-0125.json"),
        "gpt_4_have": _load_json("gpt-4_test_id/have_3.5.json"),
        "gpt_4_lack": _load_json("gpt-4_test_id/lack_3.5.json"),
        "allocation_se_co": _load_json("data/allocation_se_co.json"),
        "new_character": _load_json("data/characters_addition.json"),
        "re_check": _load_json("data/re_check.json"),
        "check_chara_question": _load_json("data/check_chara_question.json"),
    }


# Module-level placeholders populated by ``load_experiment_data`` in __main__.
all_chara = None
allocation_ex = None
allocation_se = None
need_run_ids = None
gpt_4_have = None
gpt_4_lack = None
allocation_se_co = None
new_character = None
re_check = None
check_chara_question = None
...
if __name__ == "__main__":
    # Populate module-level data references (raises if data files are missing).
    _data = load_experiment_data()
    all_chara = _data["new_character"]  # original code reassigns to additional characters
    allocation_ex = _data["allocation_ex"]
    allocation_se = _data["allocation_se"]
    need_run_ids = _data["need_run_ids"]
    gpt_4_have = _data["gpt_4_have"]
    gpt_4_lack = _data["gpt_4_lack"]
    allocation_se_co = _data["allocation_se_co"]
    new_character = _data["new_character"]
    re_check = _data["re_check"]
    check_chara_question = _data["check_chara_question"]
